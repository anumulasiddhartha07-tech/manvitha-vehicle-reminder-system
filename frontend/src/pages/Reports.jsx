...
const Reports = () => {
  ...
  const fetchAllVehicles = async () => {
    ...
    try {
      ...
      const url = `https://manvitha-vehicle-reminder-system-1.onrender.com/api/vehicles?limit=1000`; // Fetch all vehicles
  ...
  const handleExport = async (format) => {
    try {
      ...
      if (format === 'csv') {
        ...
        const url = `https://manvitha-vehicle-reminder-system-1.onrender.com/api/reports/export?${queryParams.toString()}`;
        const res = await fetch(url, {
          headers: { Authorization: `Bearer ${token}` }
        });
