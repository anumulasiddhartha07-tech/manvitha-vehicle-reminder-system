...
const Documents = () => {
  ...
  const handleReplaceSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:5000/api/documents/${replaceDocId}/replace`, {
        method: 'POST',
        headers: { 
  ...
  const executeDeleteDoc = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:5000/api/documents/${docToDelete.id}?document_type=${docToDelete.document_type}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
